...
/// #123: a duration to the millisecond: `1.713 s`, `2 min 03.456 s`,
/// `1 h 02 min 03.456 s`. Negative reads as 0.
String fmtDurationMs(int ms) {
  if (ms < 0) ms = 0;
  final frac = pad(ms % 1000, 3);
  final totalS = ms ~/ 1000;
  if (totalS < 60) return '$totalS.$frac s';
  final s = pad(totalS % 60);
  final totalMin = totalS ~/ 60;
  if (totalMin < 60) return '$totalMin min $s.$frac s';
  return '${totalMin ~/ 60} h ${pad(totalMin % 60)} min $s.$frac s';
}

/// #123: an alarm window in the raw log's own stamp format, so it can be
/// matched against it: `2026-09-21 15:06:59.329 → 15:07:01.042 · 1.713 s`.
/// The end drops its date when it falls on the start's local day; a window
/// crossing midnight shows both full stamps.
String fmtWindowMs(int startMs, int endMs) {
  ...
  final end = sameDay ? fmtStamp(b).substring(11) : fmtStamp(b);
  return '${fmtStamp(a)} → $end · ${fmtDurationMs(endMs - startMs)}';
}
