...
class DemoBattery {
  ...
  void _emitCycle() {
    ...
    onFrame(_vol(cells));
    // #114: two sensors, each sent twice, like a real pack: A = 30 °C in
    // bytes 0 and 3, B = 28 °C in byte 1 with byte 2 a degree off on odd SOC.
    onFrame(_temp(a: 30, b: 28, b2: 28 + soc % 2));
    onFrame(_allData(
      packMv: packMv,
      currentMilliA: (currentA * 1000).round(),
      load: discharging,
      charger: charging,
      chipTempC: 0, // real firmware never fills this byte (#114)
      maxMv: maxMv,
      minMv: minMv,
      avgMv: avgMv,
  ...
  /// TEMP frame: sensor [a] in bytes 0 and 3, sensor [b] in byte 1 and its
  /// second reading [b2] (within 1 °C of [b]) in byte 2 (#114).
  List<int> _temp({required int a, required int b, required int b2}) => [
        0xA1, 0x4F,
        a & 0xff, b & 0xff, b2 & 0xff, a & 0xff,
        0xB2, 0xE3,
      ];
