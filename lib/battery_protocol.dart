...
class BatteryState {
  ...
  // TEMP (A1 4F): four signed-int8 bytes p0..p3 carrying TWO sensors, each
  // reported twice (#114): p0 == p3 in every real frame (sensor A) and
  // p1 ~= p2 within 1 C (sensor B). Where the sensors sit is unknown, so the
  // UI names them "Temp A" / "Temp B", never cell temperatures.
  //
  // The field names are NOT in byte order: temp2 holds byte[3] and temp3
  // holds byte[2]. temp1/temp2 were the first pair logged (the vendor app's
  // p1/p3, the Python store's t1/t2); temp0/temp3 came later. The
  // interval-store keys temp0..temp3 are these names, so the mapping stays
  // (changing it would mix two bytes into one stored series):
  //   byte[0] -> temp0 (sensor A, copy)   byte[1] -> temp1 (sensor B) = Temp B
  //   byte[2] -> temp3 (sensor B, copy)   byte[3] -> temp2 (sensor A) = Temp A
  int? temp0; // deg C, signed (byte[0]) — sensor A, duplicate of temp2
  int? temp1; // deg C, signed (byte[1]) — sensor B: shown as "Temp B"
  int? temp2; // deg C, signed (byte[3]) — sensor A: shown as "Temp A"
  int? temp3; // deg C, signed (byte[2]) — sensor B, second reading (±1 C)

  /// Sensor A (#114): byte[3], identical to byte[0] on every real frame.
  int? get tempA => temp2 ?? temp0;

  /// Sensor B (#114): byte[1]; byte[2] reads the same sensor within 1 C.
  int? get tempB => temp1 ?? temp3;
  ...
  // BAL_STATUS (A8 AC). [chargeState] is the BMS's own s0 byte, kept verbatim
  // (the packed `flags` metric and the alarm-event rows record it as sent).
  // What the app SHOWS and signs current by is [status], not this byte.
  ChargeState chargeState = ChargeState.unknown;

  /// The pack's status as shown to the user and used to sign current / power
  /// (+ in / − out). BAL s0 alone is not enough: this firmware keeps s0 = 0
  /// ("idle") while a load draws up to ~4 A (447 of 754 real discharge frames
  /// in the 2026-09-24 merged data, e.g. 1.43 A / 19.2 W on JS-2C14B8), and
  /// only reports 2 from ~2.4 A up. "Flow" is ALL_DATA current OR power
  /// above zero (the same frame carries both; in real frames they are zero
  /// together, and a last-known record may hold only one of them). The rule,
  /// in order (docs/PROTOCOL.md, "Status rule"):
  ///  1. no ALL_DATA current or power yet → s0 as reported (unknown before
  ///     any BAL frame);
  ///  2. no flow (0.0 A and 0 W) → idle; a load / charger flag only means
  ///     something is attached;
  ///  3. flow and s0 is 1 / 2 → charging / discharging;
  ///  4. flow, s0 is 0 or unknown → the one attachment flag that is set:
  ///     charger → charging, load → discharging;
  ///  5. otherwise (flow, no direction reported) → unknown.
  ChargeState get status {
    final i = packCurrent, p = power;
    if (i == null && p == null) return chargeState;
    if ((i ?? 0) <= 0 && (p ?? 0) <= 0) return ChargeState.idle;
    if (chargeState == ChargeState.charging ||
        chargeState == ChargeState.discharging) {
      return chargeState;
    }
    final load = loadConnected == true;
    final charger = chargerConnected == true;
    if (charger && !load) return ChargeState.charging;
    if (load && !charger) return ChargeState.discharging;
...
class BatteryParser {
  ...
  // Four signed-int8 bytes, two sensors each reported twice (#114; see the
  // BatteryState fields). NOT byte order: [2] -> temp3 and [3] -> temp2 is
  // the mapping every stored temp0..temp3 row was written with. Keep it.
