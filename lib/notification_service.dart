...
class AlertNotificationService {
  ...
  /// Fleet disconnects: a NEW low-importance, silent channel. Android fixes a
  /// channel's importance / sound at creation, so the existing default-
  /// importance "alerts" channel could not be made quiet in place.
  static const connectionChannelId = 'connection';
  static const connectionChannelName = 'Connection';
  ...
  Future<void> _createChannels() async {
    ...
    await android.createNotificationChannel(const AndroidNotificationChannel(
      alertsChannelId,
      alertsChannelName,
      description: 'Status-byte changes.',
      importance: Importance.defaultImportance,
    ));
    await android.createNotificationChannel(const AndroidNotificationChannel(
      connectionChannelId,
      connectionChannelName,
      description: 'Fleet batteries that lost their Bluetooth link (silent).',
  ...
  NotificationDetails _detailsFor(AlertChannel channel) => switch (channel) {
        ...
        AlertChannel.alerts => const NotificationDetails(
            android: AndroidNotificationDetails(
              alertsChannelId,
              alertsChannelName,
              channelDescription: 'Status-byte changes.',
              importance: Importance.defaultImportance,
              priority: Priority.defaultPriority,
            ),
            iOS: DarwinNotificationDetails(),
          ),
        AlertChannel.connection => const NotificationDetails(
            android: AndroidNotificationDetails(
              connectionChannelId,
              connectionChannelName,
              channelDescription:
                  'Fleet batteries that lost their Bluetooth link (silent).',
              importance: Importance.low,
