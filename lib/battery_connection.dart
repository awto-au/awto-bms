...
class BatteryConnection {
  ...
  void _setConn(ConnState s) {
    ...
    if (s == ConnState.disconnected) {
      if (prev == ConnState.connected && !disconnectExpected) {
        _linkError = true;
        // A lost link is shown (red state, banner reason, Diagnostics) but
        // never beeps: only genuine faults and unknown-byte changes do.
        _raiseAlert('Disconnected / BLE error', beep: false);
      }
  ...
  /// Record [reason] and turn the alarm state on. [beep] false (a lost BLE
  /// link) keeps the visual alarm without the in-app beep / haptic.
  void _raiseAlert(String reason, {bool beep = true}) {
    if (alarmReasons.length > 12) alarmReasons.removeAt(0);
    alarmReasons.add(reason);
    if (beep) _pendingBeep = true;
    _recomputeAlarm();
  }
  ...
  /// latched over-temperature byte[2], a warning, nor the unknown temperature
  /// bytes [3]/[6]) active right now? Exposes the same condition that
  ...
  Future<OtaResult> runFirmwareUpdate(
    ...
    AppLog.instance.record(
        _source,
        'firmware update starting on $serial: link mtu '
    ...
    try {
      ...
      AppLog.instance.record(
          _source,
          'firmware update on $serial ended: ${session.stage.name}'
  ...
  /// Also null while current flows with no direction reported
  /// ([BatteryState.status] unknown): "0.0 A" there would be false.
  double? get signedCurrentOrNull {
    final i = state.packCurrent;
    if (i == null) return null;
    if (i > 0 && state.status == ChargeState.unknown) return null;
  ...
  /// #91: the reported state of a single-byte gate toggle — low-temp
  /// protection (byte[2]), smoke sensor (byte[3]), heater (byte[4]) or
  /// passive balancing (byte[6]) — or null when BAL_STATUS has not reported
  /// it on this row. Never defaults an unknown gate to off.
  bool? gateState(GateAction action) => switch (action) {
        GateAction.tempControlGate =>
          state.tempControlGate == null ? null : state.tempControlGate != 0,
        GateAction.smokeGate =>
          state.smokeGate == null ? null : state.smokeGate != 0,
        GateAction.heatGate =>
          state.heatGate == null ? null : state.heatGate != 0,
        GateAction.passiveBalance => state.passiveBalancing,
        _ => null,
      };

  /// #91: low-temperature protection (gate byte[2]) as last reported.
  bool? get lowTempProtectionOn => gateState(GateAction.tempControlGate);

  /// #91: the smoke-sensor gate (byte[3]) as last reported.
  bool? get smokeSensorOn => gateState(GateAction.smokeGate);

  /// #91: read-back for a single-byte gate toggle ([gateState] actions).
  /// Completes `true` as soon as a decoded frame shows THAT gate at
  /// [expectedOn], or `false` if it has not within [timeout] — the next
  /// BAL_STATUS after the write is what confirms it.
  Future<bool> confirmGateState(
    GateAction action,
    bool expectedOn, {
    Duration timeout = const Duration(seconds: 4),
  }) =>
      _confirmVia(() => gateState(action) == expectedOn, timeout);
  ...
  Future<void> turnSwitchOff(GateAction action) async {
    ...
    if (needsStandbyOffFirst) {
      ...
      AppLog.instance.record(
          _source,
          'Bluetooth standby $was on $serial: turning standby OFF before '
  ...
  Future<void> _runProbe(Completer<StreamClass> c) async {
    ...
    try {
      ...
      await _send(BatteryCommands.getVersion, label: 'AT+V probe');
  ...
  Future<bool> resendWake({Duration timeout = resumeTimeout}) async {
    ...
    AppLog.instance.record(
        _source, 'wake ladder: re-send wake (CMD_BEGIN) to ${state.serial}');
    await _send(BatteryCommands.begin, label: 'wake (CMD_BEGIN, wake ladder)');
    return resumed;
  }
  ...
  Future<bool> switchesOnToWake({Duration timeout = resumeTimeout}) async {
    final resumed = awaitStreaming(timeout: timeout);
    AppLog.instance.record(
        _source, 'wake ladder: both switches ON to ${state.serial}');
  ...
  /// Direction is [BatteryState.status], not the raw BAL s0 byte: the BMS
  /// reports s0 = 0 through low-current discharge, which used to zero the
  /// signed current (and the last-known current) while power still showed.
  /// A flow with no direction reported signs as 0.
  ...
  /// Power signed by direction: + in, - out ([BatteryState.status]).
  ...
  void _logDecoded(BatteryEvent e) {
    ...
    final detail = switch (e) {
      ...
      // #114: two sensors, each sent twice. Named A/B, with the four bytes
      // in BYTE order (p0 p1 p2 p3 = temp0 temp1 temp3 temp2 — the old
      // "t0..t3" text followed the field names, so t2/t3 read as swapped).
      TempEvent() => 'A=${s.tempA}C B=${s.tempB}C '
          'p0..p3=${s.temp0}/${s.temp1}/${s.temp3}/${s.temp2}',
      AllDataEvent() => 'V=${s.packVoltage} I=${s.packCurrent}A '
          'P=${s.power}W sum=${s.cellSum} max=${s.cellMax} min=${s.cellMin} '
          'diff=${s.cellDiff} avg=${s.cellAvg} chipByte=${s.chipTemperature} '
