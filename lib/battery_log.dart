...
class LoggerAttachment implements ConnectionAttachment {
  ...
  /// The logger this handle feeds.
  BatteryLogger get logger => _logger;
...
/// #111: what [BatteryLogger.deleteHistory] removed. [done] is false when the
/// store was not open or the delete failed (nothing was removed then).
class HistoryDeletion {
  final bool done;
  final int readings;
  final int alarmEvents;
  const HistoryDeletion({required this.readings, required this.alarmEvents})
      : done = true;
  const HistoryDeletion.notDone()
      : done = false,
        readings = 0,
        alarmEvents = 0;
}
...
class BatteryLogger {
  ...
  /// #112: attach [conn] unless it is already attached to THIS logger. Every
  /// path that links a pack calls this, so a row can never stream to the UI
  /// and the raw log without also feeding the store (a remembered pack first
  /// linked by a background sample used to stay unattached for the whole
  /// process). Idempotent: a live attachment is kept, not re-subscribed.
  LoggerAttachment ensureAttached(BatteryConnection conn) {
    final current = conn.loggerAttachment;
    if (current is LoggerAttachment &&
        !current.isCancelled &&
        identical(current.logger, this)) {
      return current;
    }
    return attach(conn);
  }

  /// True while [conn] feeds this logger.
  bool isAttached(BatteryConnection conn) {
    final current = conn.loggerAttachment;
    return current is LoggerAttachment &&
        !current.isCancelled &&
        identical(current.logger, this);
  }
  ...
  void _openNew(String key, String serial, String metric, int now,
    ...
    _enqueue('insert $metric', (db) async {
      await db.insert(_kReadings, {
        'id': id,
        'serial': serial,
        'metric': metric,
        'value_num': valueNum,
  ...
  void _finalize(_Open open) {
    ...
    _enqueue('update end ${open.metric}', (db) async {
      await db.update(_kReadings, {'end_time': fmtTime(end), 'end_ms': end},
          where: 'id = ?', whereArgs: [id]);
      _noteRowWrite(inserted: false);
    });
  }
  ...
  // --- #112: write health for Diagnostics ---------------------------------

  /// Epoch-ms of the last `readings` write that landed (a new row or an
  /// advanced end), or null if none has landed since the app started.
  int? get lastWriteMs => _lastWriteMs;
  int? _lastWriteMs;

  /// `readings` rows inserted since the app started.
  int get rowsWrittenThisSession => _rowsWritten;
  int _rowsWritten = 0;

  void _noteRowWrite({required bool inserted}) {
    _lastWriteMs = DateTime.now().millisecondsSinceEpoch;
    if (inserted) _rowsWritten++;
  }

  /// The Diagnostics line: `History last saved: 14:11:32 · 53 rows this
  /// session`.
  String historySavedLine({int? nowMs}) => historySavedText(
      lastWriteMs: _lastWriteMs, rows: _rowsWritten, nowMs: nowMs);

  /// Wait for every queued write to land (tests).
  @visibleForTesting
  Future<void> drain() => _dbChain;

  /// On-disk path of the store once [init] has resolved it (null for a
  /// memory-only logger).
  String? get dbPath => _dbPath;
  String? _dbPath;

  /// #104: write a consistent copy of the store to [dest] for a data export.
  /// Open intervals are flushed first and the copy runs on the write queue, so
  /// no write of ours lands half-way through it. `VACUUM INTO` gives a
  /// compacted, self-contained file; SQLite older than 3.27 (old Android)
  /// lacks it, so the fallback is a plain file copy. Returns false when the
  /// store is not open or both methods fail.
  Future<bool> snapshotTo(String dest) {
    final db = _db;
    final src = _dbPath;
    ...
    _dbChain = _dbChain.then((_) async {
      ...
      try {
        ...
        logLine(_source, 'VACUUM INTO failed, copying the file instead: $e');
        try {
          if (src == null) throw StateError('no database path');
          await File(src).copy(dest);
          done.complete(true);
        } catch (e2) {
          logLine(_source, 'snapshot failed: $e2');
          done.complete(false);
        }
      }
    });
  ...
  /// #111: bytes the store uses on disk (the database file plus its SQLite
  /// side files). 0 for a memory-only store or when the size cannot be read.
  Future<int> storageBytes() async {
    ...
    for (final suffix in const ['', '-journal', '-wal', '-shm']) {
      try {
        ...
        // best effort: an unreadable side file counts as 0
      }
    }
    return total;
  }

  /// #111: delete stored history at the USER'S explicit request (the "Keep
  /// data" period or a one-off delete in Settings → Data). Nothing calls this
  /// on its own. With [beforeMs] only `readings` whose `end_ms` is older, and
  /// `alarm_events` whose `at_ms` is older, are deleted; with null every
  /// reading and alarm event is. `lifetime_totals` is never touched: every
  /// serial's pack current is folded into its totals FIRST, so the totals do
  /// not change when rows go. Runs on the write queue, so it cannot race a
  /// logging insert/update. [compact] runs `VACUUM` afterwards so the file
  /// actually shrinks (used for one-off deletes).
  Future<HistoryDeletion> deleteHistory({int? beforeMs, bool compact = false}) {
    final db = _db;
    if (db == null) return Future.value(const HistoryDeletion.notDone());
    _flushOpens(); // queued ahead of the delete, so rows hold their true end
    final done = Completer<HistoryDeletion>();
    _dbChain = _dbChain.then((_) async {
      if (_db != db) {
        done.complete(const HistoryDeletion.notDone());
        return;
      }
      try {
        final serials =
            await db.rawQuery('SELECT DISTINCT serial FROM $_kReadings');
        for (final r in serials) {
          await updateLifetimeTotals('${r['serial']}');
        }
        final readings = beforeMs == null
            ? await db.delete(_kReadings)
            : await db.delete(_kReadings,
                where: 'end_ms < ?', whereArgs: [beforeMs]);
        final alarms = beforeMs == null
            ? await db.delete(_kAlarms)
            : await db.delete(_kAlarms,
                where: 'at_ms < ?', whereArgs: [beforeMs]);
        if (compact) await db.execute('VACUUM');
        // An open interval whose row was deleted starts a fresh row on its
        // next reading instead of updating a row that no longer exists.
        if (beforeMs == null) {
          _open.clear();
        } else {
          _open.removeWhere((_, o) => o.flushedEndMs < beforeMs);
        }
        // Re-read the alarm tallies from what is left.
        _alarmStats.clear();
        _alarmStatsSeeded.clear();
        _noteSuccess();
        logLine(
            _source,
            'deleted $readings readings, $alarms alarm events '
            '${beforeMs == null ? '(all)' : 'ending before ${fmtTime(beforeMs)}'}');
        done.complete(HistoryDeletion(readings: readings, alarmEvents: alarms));
      } catch (e) {
        _noteFailure('delete history', e);
        done.complete(const HistoryDeletion.notDone());
      }
    });
    return done.future;
  ...
  /// #110: one [HistorySummary] per serial with stored history — its latest
  /// SOC, pack voltage, remaining and full Ah and when they were last logged —
  /// used ONCE to seed the "known batteries" list. Read-only; empty when the
  /// store is not open. A serial whose readings the user deleted (#111) but
  /// whose lifetime totals remain is still listed.
  Future<List<HistorySummary>> historySummaries() async {
    final db = _db;
    if (db == null) return const [];
    final rows = await db.rawQuery('SELECT DISTINCT serial FROM $_kReadings '
        'UNION SELECT serial FROM $_kLifetime');
    final out = <HistorySummary>[];
    for (final r in rows) {
      final serial = r['serial'];
      if (serial is! String || serial.isEmpty) continue;
      final latest = <String, double>{};
      int? lastMs;
      for (final metric in HistorySummary.metrics) {
        final v = await db.rawQuery(
            'SELECT value_num, end_ms FROM $_kReadings '
            'WHERE serial = ? AND metric = ? '
            'ORDER BY start_ms DESC, id DESC LIMIT 1',
            [serial, metric]);
        if (v.isEmpty) continue;
        final num? value = v.first['value_num'] as num?;
        final end = (v.first['end_ms'] as num?)?.toInt();
        if (value != null) latest[metric] = value.toDouble();
        if (end != null && (lastMs == null || end > lastMs)) lastMs = end;
      }
      out.add(HistorySummary(
        serial: serial,
        lastMs: lastMs,
        soc: latest[Metric.soc]?.round(),
        packVoltage: latest[Metric.packVoltage],
        remainingAh: latest[Metric.remainingAh],
        fullAh: latest[Metric.fullAh],
      ));
    }
    return out;
  }
  ...
  /// `readings` rows are read-only here (only the user's explicit delete,
  /// [deleteHistory], removes rows, and it folds first). On first run
...
/// #112: the Diagnostics line that makes a silent store gap visible:
/// `History last saved: 14:11:32 · 53 rows this session` (the time alone when
/// it is today, `23 Sep 11:24:04` otherwise; `never` before the first write).
/// Pure.
String historySavedText({int? lastWriteMs, required int rows, int? nowMs}) {
  final String when;
  if (lastWriteMs == null) {
    when = 'never';
  } else {
    final t = DateTime.fromMillisecondsSinceEpoch(lastWriteMs);
    final n = DateTime.fromMillisecondsSinceEpoch(
        nowMs ?? DateTime.now().millisecondsSinceEpoch);
    final today = t.year == n.year && t.month == n.month && t.day == n.day;
    when = today
        ? '${pad(t.hour)}:${pad(t.minute)}:${pad(t.second)}'
        : fmtShortStamp(lastWriteMs);
  }
  return 'History last saved: $when · $rows ${rows == 1 ? 'row' : 'rows'}'
      ' this session';
}
...
/// #110: the latest summary values stored for one serial
/// ([BatteryLogger.historySummaries]); null = never logged.
class HistorySummary {
  /// The metrics a summary reads (the list card's figures).
  ...
  /// Epoch-ms the newest of those values was last logged.
  final int? lastMs;
  final int? soc;
