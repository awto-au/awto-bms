...
/// The pack's direction for colours and words: [BatteryState.status] (BAL s0
/// corrected by the ALL_DATA current and flags; see docs/PROTOCOL.md "Status
/// rule"). Never `unknown`: a pack with no direction reported styles as idle.
ChargeState effState(BatteryState s) {
  final st = s.status;
  return st == ChargeState.unknown ? ChargeState.idle : st;
}
