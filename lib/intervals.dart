...
///  * [seriesStats] — current / min / max / time-weighted median and average
///    of a series
///    over a window (#70).
...
class SeriesStats {
  ...
  /// #117: TIME-WEIGHTED average (see [seriesStats]).
  ...
  @override
  String toString() =>
      'SeriesStats(now $current min $min max $max med $median avg $average, '
      '$rows rows)';
}
...
///
/// #117: the AVERAGE is time-weighted the same way: sum(value x clipped held
/// time) / sum(clipped held time). Only the held rows carry weight, so an
/// offline gap (no row covers it) adds nothing and is never averaged across.
/// With zero total held time it falls back to the plain mean of the values.
SeriesStats? seriesStats(
  ...
  var total = 0;
  var weightedSum = 0.0, plainSum = 0.0;
  ...
  final average =
      total > 0 ? weightedSum / total : plainSum / weighted.length;
  double median;
  if (total <= 0) {
