...
List<Object?> listPageSignature(
  ...
  for (final b in m.batteries) {
    ...
    sig.addAll([
      ...
      b.inFleet,
      m.isNewThisSession(s.serial), // #110: the row's list group
      b.alarmActive,
...
class AppSession extends ChangeNotifier with WidgetsBindingObserver {
  ...
  /// #111: "Keep data" (persisted, default All = nothing is ever deleted) and
  /// the job that applies a chosen period at startup and daily.
  KeepData keepData = KeepData.all;
  final RetentionJob retention = RetentionJob();
  ...
  AppSession({
    ...
    AliasStore? aliases,
            ...
            manager ??
            BatteryManager(
                fleetStore: SharedPrefsFleetStore(),
                knownStore: SharedPrefsKnownStore()),
  ...
  /// starting, so the demo-mode choice is honoured and each pack
  ...
  Future<void> _restoreThenStart() async {
    ...
    // The raw log is always on (user, 2026-09-24): a log switched off by
    // mistake hid 23 h of frames, including the event that put a pack to
    // sleep. A stale `verbose_logging_v1 = false` from older builds is ignored.
    ...
    // #111: apply a chosen "Keep data" period now and daily; All does nothing.
    keepData = KeepData.fromMonths(await settings.loadKeepDataMonths());
    if (!keepData.keepsAll) unawaited(retention.configure(keepData));
  ...
  void _tick() {
    // Beep loudly the moment any battery raises a new alert (fault or
    // unknown-byte change; a lost link never beeps). Edge-triggered via
    // consumeBeep().
    var beep = false;
    for (final b in manager.batteries) {
  ...
  /// #111: Settings "Keep data" (persisted). The page has already asked for
  /// confirmation of a non-default period; it is applied at once and daily.
  Future<DataDeletion?> setKeepData(KeepData keep) async {
    keepData = keep;
    notifyListeners();
    await settings.saveKeepDataMonths(keep.months);
    return retention.configure(keep);
  }
  ...
  /// #110: forget [b] (the caller has confirmed): its row is hidden, its
  /// history is kept. See [BatteryManager.forgetBattery].
  void forgetBattery(BatteryConnection b) {
    manager.forgetBattery(b);
    notifyListeners();
  }
