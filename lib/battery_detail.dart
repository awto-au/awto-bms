...
class BatteryDetailView extends StatefulWidget {
  ...
  /// Desktop density: smaller figures, status + values on one line (card
  /// padding is the one compact [kCardPadding] on both layouts, #3).
  final bool dense;
...
class _BatteryDetailViewState extends State<BatteryDetailView> {
  ...
  /// #123: the packed `flags` history over the same window — the alarm band
  /// in the Alarm events section.
  List<ReadingInterval> _flags = const [];
  int _sparkFrom = 0;
  int _sparkTo = 0;
  Timer? _sparkTimer;
  ...
  Future<void> _loadSpark() async {
    ...
    try {
      ...
      final flags = // #123: the alarm band
          await log.mergedSeries(serial, Metric.flags, sinceMs: since);
      if (!mounted) return;
  ...
  Widget _alarmEvents() {
    final s = widget.conn.state;
    // #67: every alarm-byte transition with its snapshot; #123: as set ->
    // clear windows under a band over the Trends window.
  ...
  Widget _controls() => ControlsSection(
        ...
        keepAwake: widget.keepAwake, // firmware update check
        dense: widget.dense,
      );
  ...
  Widget build(BuildContext context) {
    ...
    return switch (widget.arrangement) {
      // #3: both arrangements use the one compact page padding and no
      // spacer rows between sections (the cards' margins make the 4 px gap).
      DetailArrangement.stacked => ListView(
          ...
          children: [
            ...
            ..._warnings(),
            _alarmEvents(),
            // Controls carries the firmware update (#118), never the list.
            _controls(),
