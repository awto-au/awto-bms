...
///
/// #123: the fault / alarm timeline moved to the battery detail page (the
/// Alarm events section), next to the per-bit records with ms timestamps.
library;
...
class _BatteryChartsPageState extends State<BatteryChartsPage> {
  ...
  ({String title, String unit, double? minY, double? maxY}) _spec(
      ...
      switch (g) {
        ...
        ChartGroup.packVoltage => (title: 'Voltage', unit: 'V', minY: null, maxY: null),
        ...
        ChartGroup.soc => (title: 'SOC', unit: '%', minY: 0, maxY: 100),
      };
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            // #70/#117: now / min / max / median / average for the window, per
...
/// data in the window, ONE line `now · min · max · med · avg` (#117) in the
/// series colour (with the series label first when the card has several),
/// scaled down rather than wrapped on a narrow screen. Values go through the
...
/// #70/#117: the readout text for one series' stats, through the card's value
/// [transform] (°F), in the order min, max, median, average after `now`:
/// `now 3.312 · min 3.298 · max 3.320 · med 3.310 · avg 3.309`. Pure. (°F is
/// linear, so transforming the °C average equals averaging in °F.)
String statsText(SeriesStats st, [double Function(double)? transform]) {
  double t(double v) => transform == null ? v : transform(v);
  return 'now ${fmtStat(t(st.current))} · min ${fmtStat(t(st.min))} · '
      'max ${fmtStat(t(st.max))} · med ${fmtStat(t(st.median))} · '
      'avg ${fmtStat(t(st.average))}';
}
