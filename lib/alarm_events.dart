...
class AlarmEvent {
  ...
  /// The short snapshot phrases of a display line: current, voltage, MOS and
  /// the last command, e.g. `0.0 A`, `13.2 V`, `MOS off`,
  /// `0.5 s after 'Output ON'`.
  List<String> get snapshotParts => [
        fSignedA(packI),
  ...
  String snapshotText() {
    ...
    return "$frame byte $byteIndex '$bitName' $fromValue -> $toValue "
        ...
        // #114: Temp A = temp2, Temp B = temp1 (the duplicates and the
        // always-0 chip byte stay in the record, not in this line).
        'tempA=${t(temp2)} tempB=${t(temp1)} '
...
/// #123: one alarm WINDOW — a bit's `set` row paired with the `cleared` row
/// that ended it — the unit the detail page's alarm timeline lists. Either
/// half can be missing: a bit still set (or whose clear was never recorded)
/// has no [cleared]; a clear whose `set` row is not in the loaded page has no
/// [set], and its start is then derived from the clear's duration when known.
class AlarmWindow {
  final AlarmEvent? set;
  final AlarmEvent? cleared;
  const AlarmWindow({this.set, this.cleared})
      : assert(set != null || cleared != null);

  AlarmEvent get _any => (set ?? cleared)!;
  String get bitName => _any.bitName;

  /// When the bit was set (null: unknown — a clear with no duration).
  int? get startMs {
    final s = set;
    if (s != null) return s.atMs;
    final c = cleared!;
    return c.durationMs == null ? null : c.atMs - c.durationMs!;
  }

  /// When the bit cleared (null: still set, or no clear recorded).
  int? get endMs {
    final c = cleared;
    if (c != null) return c.atMs;
    final s = set!;
    return s.durationMs == null ? null : s.atMs + s.durationMs!;
  }

  /// Still set as far as the record knows (no clear, no duration).
  bool get open => endMs == null;

  /// Newest-first sort key: the start, else the end.
  int get sortMs => startMs ?? endMs ?? 0;

  /// The row text, full local date-time to the ms (the raw log's format):
  /// `2026-09-21 15:06:59.329 → 15:07:01.042 · 1.713 s  Short circuit
  /// protection — 0.0 A · 13.2 V · MOS off · 0.5 s after 'Output ON'`. The
  /// snapshot is the set row's (the moment it tripped), else the clear's.
  String describe() {
    final s = startMs, e = endMs;
    final String when;
    if (s != null && e != null) {
      when = fmtWindowMs(s, e);
    } else if (s != null) {
      when = '${fmtStampMs(s)} → no clear recorded';
    } else {
      when = 'set before the record → ${fmtStampMs(e!)}';
    }
    return '$when  $bitName — ${_any.snapshotParts.join(' · ')}';
  }

  /// Both rows' full snapshots (the tooltip).
  String snapshotText() => [
        if (set != null) set!.snapshotText(),
        if (cleared != null) cleared!.snapshotText(),
      ].join('\n');
}

/// #123: pair [events] (any order) into [AlarmWindow]s, NEWEST first. A
/// `set` opens a window for its frame + byte; the next `cleared` of the same
/// byte closes it. A second `set` of a byte already open (a non-zero value
/// changing to another) leaves the earlier window without a clear, as the
/// store does. Pure.
List<AlarmWindow> alarmWindows(List<AlarmEvent> events) {
  final ordered = [...events]..sort((a, b) {
      final c = a.atMs.compareTo(b.atMs);
      return c != 0 ? c : (a.id ?? 0).compareTo(b.id ?? 0);
    });
  final out = <AlarmWindow>[];
  final open = <String, int>{}; // frame|byte -> index in out
  for (final e in ordered) {
    final key = '${e.frame}|${e.byteIndex}';
    if (e.isSet) {
      open[key] = out.length;
      out.add(AlarmWindow(set: e));
    } else {
      final i = open.remove(key);
      if (i != null) {
        out[i] = AlarmWindow(set: out[i].set, cleared: e);
      } else {
        out.add(AlarmWindow(cleared: e));
      }
    }
  }
