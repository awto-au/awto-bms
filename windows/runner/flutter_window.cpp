...
// Fit [r] inside [work]: shrink it to the work area if it is larger, then
// shift it so no edge hangs off (e.g. saved on a taller monitor or at a
// different DPI, which left the window mostly below the screen).
RECT ClampToWorkArea(RECT r, const RECT& work) {
  const LONG w = (std::min)(r.right - r.left, work.right - work.left);
  const LONG h = (std::min)(r.bottom - r.top, work.bottom - work.top);
  r.left = (std::max)(work.left, (std::min)(r.left, work.right - w));
  r.top = (std::max)(work.top, (std::min)(r.top, work.bottom - h));
  r.right = r.left + w;
  r.bottom = r.top + h;
  return r;
}
      ...
      [this](const flutter::MethodCall<flutter::EncodableValue>& call,
        ...
        if (method == "getBounds") {
          ...
          const HMONITOR monitor =
              ::MonitorFromRect(&r, MONITOR_DEFAULTTONULL);
          if (monitor == nullptr) {
            result->Success(flutter::EncodableValue(false));
            return;
          }
          // #106: partly on screen is not enough — pull it fully inside the
          // monitor's work area (above the taskbar).
          MONITORINFO info = {};
          info.cbSize = sizeof(info);
          if (::GetMonitorInfo(monitor, &info)) {
            r = ClampToWorkArea(r, info.rcWork);
          }
