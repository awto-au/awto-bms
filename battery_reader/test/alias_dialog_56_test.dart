...
void main() {
  ...
  group('#56 the store tolerates null / empty / blank', () {
    ...
    test('an unavailable prefs plugin: clear still updates the cache and '
  ...
  group('#56 on the real list card and detail page (demo fleet)', () {
    testWidgets('card: clear via the pencil updates the card at once, no toast',
      ...
      SharedPreferences.setMockInitialValues({
        'demo_mode_v1': true,
        'battery_aliases_v1': '{"JS-2C14AA":"Left"}',
      });
      await tester.pumpWidget(const BatteryReaderApp());
      await pumpFor(tester, const Duration(seconds: 3));
      expect(find.textContaining('Left (JS-2C14AA)'), findsOneWidget);
      ...
      await pumpThroughTransition(tester);
      _expectClean(tester);
      expect(find.textContaining('Left (JS-2C14AA)'), findsNothing);
    ...
    testWidgets('detail page: title follows clear / rename immediately',
      ...
      await pumpFor(tester, const Duration(seconds: 1));
      expect(find.text('Left ($serial)'), findsOneWidget);
      await tester.tap(find.byIcon(Icons.edit).first);
      await pumpFor(tester, const Duration(seconds: 1));
      await tester.enterText(find.byType(TextField), '   ');
      await tester.tap(find.text('Save'));
      await pumpThroughTransition(tester);
      _expectClean(tester);
      expect(find.text('Left ($serial)'), findsNothing);
      ...
      await pumpThroughTransition(tester);
      _expectClean(tester);
      expect(find.text('Right ($serial)'), findsOneWidget);
      manager.disposeAll();
      await tester.pump();
    });
  });
