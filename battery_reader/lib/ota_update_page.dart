/// Firmware update UI (#41): pick file -> pre-flight checklist -> stern
/// warning -> typed serial confirmation -> progress (no back navigation,
/// Abort only before the first chunk) -> result.
///
...
enum _Step { pick, preflight, warning, confirm, running, result }
...
class _FirmwareUpdatePageState extends State<FirmwareUpdatePage> {
  ...
  Future<void> _pick() async {
    ...
    try {
      ...
      final image = OtaImage(name: f.name, bytes: bytes);
  ...
  Widget _stepper() {
    const names = ['File', 'Checks', 'Warning', 'Confirm', 'Flash', 'Result'];
  ...
  Widget _warningCard() {
    return _card('3. Read this before continuing', [
      Container(
        ...
        child: const Row(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Icon(Icons.dangerous, color: _red),
            SizedBox(width: 10),
            Expanded(
      ...
      Text(
        'During the update every other command to any battery, Pause, Exit '
        'and background sampling are disabled. The transfer takes roughly '
        '${_estimate()} at the current link settings. Keep the phone next '
        'to the battery, plugged in, with the screen on.',
  ...
  String _estimate() {
    ...
    // 50–150 ms range. A rough upper bound is fine for the warning.
  ...
  Widget _confirmCard() {
    ...
    return _card('4. Type the battery serial to confirm', [
      _kv('Battery', _serial),
      ...
      Row(
        children: [
          OutlinedButton(
            onPressed: () => setState(() => _step = _Step.warning),
            child: const Text('Back'),
          ),
          const Spacer(),
          FilledButton(
  ...
  Widget _runningCard() {
    ...
    return _card('5. Updating — do not leave this screen', [
  ...
  Widget _resultCard() {
    final r = _result;
    final s = _session;
    if (r == null) return _card('6. Result', const [Text('—')]);
    final ok = r.ok;
    final color = ok
        ? _green
