...
void main() {
    ...
    testWidgets(
      ...
      for (final s in DetailSection.values) {
        ...
        expect([for (final r in phoneRows[s]!) r.$1],
            [for (final r in detailRows(s)) r.label],
  ...
  group('#68 AdaptiveScaffold: one shell, two arrangements', () {
    ...
    testWidgets('1280 px: two panes, the same row / footer / section widgets '
      ...
      await tester.tap(find.textContaining('DEMO 2').first);
      await pumpFor(tester, const Duration(seconds: 1));
      expect(find.byType(BatteryDetailPage), findsNothing);
      expect(
          tester
      ...
      expect(shell.selected?.state.serial, 'DEMO-2');
      // Down / Up move the selection.
      await tester.sendKeyEvent(LogicalKeyboardKey.arrowDown);
      await pumpFor(tester, const Duration(milliseconds: 500));
      expect(shell.selected?.state.serial, 'DEMO-3');
      await tester.sendKeyEvent(LogicalKeyboardKey.arrowUp);
      await tester.sendKeyEvent(LogicalKeyboardKey.arrowUp);
      await pumpFor(tester, const Duration(milliseconds: 500));
      expect(shell.selected?.state.serial, 'DEMO-1');
      await tester.sendKeyEvent(LogicalKeyboardKey.arrowUp);
      await pumpFor(tester, const Duration(milliseconds: 300));
      expect(shell.selected?.state.serial, 'DEMO-1', reason: 'clamped');
      expect(tester.takeException(), isNull);
      ...
      await tester.scrollUntilVisible(find.textContaining('DEMO 4'), 100,
          scrollable: find.byType(Scrollable).first);
      await tester.pump();
      await tester.tap(find.textContaining('DEMO 4').first);
      await pumpFor(tester, const Duration(seconds: 1));
      expect(shell.tab, DesktopTab.detail);
      expect(shell.selected?.state.serial, 'DEMO-4');
      expect(tester.takeException(), isNull);
      await tester.pumpWidget(const SizedBox());
      await tester.pump();
    });
