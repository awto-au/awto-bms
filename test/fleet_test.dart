...
void main() {
  ...
  group('offline favourites + bind-on-discovery (#34)', () {
    ...
    test('an unremembered discovery creates a fresh, non-fleet row', () async {
      ...
      // #110: every pack found is a known battery (remembered, not starred).
      expect(c.isRemembered, isTrue);
      expect(man.knownBatteries.keys, ['JS-NEW']);
      expect(man.isNewThisSession('JS-NEW'), isTrue);
    });
    ...
    test('un-starring an offline placeholder drops its fleet record only',
        () async {
      // #110: the star is only the fleet flag — the row stays (a known
      // battery, offline, with its last-known values).
      final store = FakeFleetStore()
      ...
      expect(man.batteries, [placeholder], reason: 'row kept on un-star');
      expect(placeholder.isOffline, isTrue);
      expect(placeholder.state.fullAh, 100);
      expect(man.fleetSerials, isEmpty);
      expect(store.saved, isEmpty);
      expect(man.knownBatteries['JS-OFF']?.record.fullAh, 100);
    });
