...
void main() {
  ...
  group('fleet panel: Status / Switches from last-known when nothing is live',
    ...
    testWidgets('Status and Switches read last-known in the stale style with '
      ...
      expect(find.text('Charging'), findsOneWidget);
      // #103: Status and Switches are one folded line under the bar.
      final status = tester.widget<Text>(find.byKey(const Key('fleet-status')));
      expect(status.data, 'Charging');
      expect(status.style!.color, kStale);
      final switches =
          tester.widget<Text>(find.byKey(const Key('fleet-switches')));
      expect(switches.style!.color, kStale);
      expect(switches.data, 'Charge 1/1 on  ·  Output 0/1 on');
