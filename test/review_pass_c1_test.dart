...
void main() {
  ...
  group('H2: schema v3 migration backfills the ms columns from the text', () {
    ...
    test('a fresh database is created at v3 with both column families',
      ...
      expect(
          names, containsAll(['start_time', 'end_time', 'start_ms', 'end_ms']));
      final lt =
          await log.debugDb!.rawQuery('PRAGMA table_info(lifetime_totals)');
  ...
  group('H2: ordering, windows and watermarks use ms, never the text', () {
    ...
    test('intervals() come back in ms order across the repeated hour',
      ...
      final late =
          await log.intervals('JS-A', Metric.soc, sinceMs: b.ms + 6 * min);
  ...
  group('Diagnostics: AppLog ring buffer + guard', () {
    ...
    test('guard passes a success straight through without recording', () async {
    ...
    test('guard applies the timeout and records it', () async {
      ...
      expect(
          AppLog.instance.entries.single.message, contains('TimeoutException'));
    });
  ...
  group('PrefsStore base: thin subclasses, unchanged keys, recorded failures',
    ...
    test('SettingsStore is a table of BoolSetting with the ORIGINAL keys', () {
      ...
      // The raw log is always on (2026-09-24): no verbose switch any more.
      expect(SettingsStore.all.map((s) => s.key),
          isNot(contains('verbose_logging_v1')));
    });

    test('data persisted under the old keys still loads', () async {
      SharedPreferences.setMockInitialValues({
        'demo_mode_v1': true,
        'verbose_logging_v1': false, // stale key from older builds: ignored
    ...
    test('round trip through the base writes the same keys', () async {
      ...
      await s.save(SettingsStore.useFahrenheit, true);
      final p = await SharedPreferences.getInstance();
      expect(p.getBool('demo_mode_v1'), isTrue);
      expect(p.getBool('temp_fahrenheit_v1'), isTrue);
      expect(await s.load(SettingsStore.useFahrenheit), isTrue);
    });
    ...
    test('an unavailable plugin falls back to defaults AND records', () async {
      ...
      expect(await s.loadAlertNotifications(), isTrue);
      ...
      expect(
          sources, containsAll(['SettingsStore', 'AliasStore', 'FleetStore']));
      expect(
          AppLog.instance.entries.every((e) => e.message.contains('Missing')),
          isTrue);
    });
  ...
  group('M7 / L18: DB failures are counted, degrade after 5 and retry', () {
    ...
    test('5 consecutive write failures flip dbDegraded; a re-open recovers',
      ...
      expect(log.consecutiveFailures, 0);
      final rows =
          await log.debugDb!.query('readings', where: 'value_num = 51');
  ...
  // -------------------------------------------------------------------------
  group('M9 + #111: raw log rotates past the size cap and keeps every file',
      () {
    test('exceeding maxBytes renames the file with its time; none is deleted',
        () async {
      ...
      var now = DateTime(2026, 9, 24, 14, 5, 1);
      raw.clock = () => now;
      ...
      expect(raw.sizeBytes, 0);
      final first = File(at('battery_raw.20260924-140501.log'));
      expect(await first.exists(), isTrue);
      expect(await first.readAsString(), contains('bb bb'));
      expect(await File(raw.path!).exists(), isFalse, reason: 'fresh start');

      // A second rotation gets its OWN file; the first is kept (#111).
      ...
      final second = File(at('battery_raw.20260925-090000.log'));
      expect(await second.readAsString(), contains('cc cc'));
      expect(await first.readAsString(), contains('bb bb'),
          reason: 'the earlier rotation is never overwritten');

      // A third rotation in the same second gets a -2 suffix, not a clash.
      for (var i = 0; i < 4; i++) {
        raw.logRaw('JS-A', List.filled(20, 0xDD));
      }
      await raw.flush();
      expect(raw.rotations, 3);
      expect(await File(at('battery_raw.20260925-090000-2.log')).readAsString(),
          contains('dd dd'));
      expect(await second.readAsString(), contains('cc cc'));

      final kept = await raw.rotatedLogs();
      expect(kept.length, 3);
      expect(raw.rotatedCount, 3);
      expect(raw.rotatedBytes, kept.fold<int>(0, (a, r) => a + r.bytes));
      // Writes continue into a new live file.
      raw.logRaw('JS-A', [1]);
      await raw.flush();
      expect(await File(raw.path!).exists(), isTrue);
      expect(raw.sizeBytes, greaterThan(0));
      expect(raw.totalBytes, raw.sizeBytes + raw.rotatedBytes);
      await raw.dispose();
    });
