...
void main() {
  ...
  group('AliasStore persistence', () {
    ...
    test('empty (or blank) clears the alias back to the serial', () async {
      ...
      final s2 = AliasStore();
      await s2.load();
      expect(s2.aliasFor('JS-A'), isNull,
          reason: 'cleared alias stays cleared');
    });
  ...
  group('displayName helper', () {
    ...
    test('an alias shows alongside the serial in parentheses', () {
      expect(
          displayName('Left battery', 'JS-2C14AA'), 'Left battery (JS-2C14AA)');
    });
    test('null/empty serial falls back to an em dash', () {
      expect(displayName(null, null), '—');
      expect(displayName('Left', null), 'Left (—)');
    });
    test('a demo pack is always labelled as test data (#115)', () {
      expect(displayName(null, 'DEMO-1'), 'DEMO 1 — test data');
      expect(displayName('', 'DEMO-4'), 'DEMO 4 — test data');
      expect(displayName('Left', 'DEMO-1'), 'DEMO 1: Left — test data');
      expect(isDemoSerial('DEMO-2'), isTrue);
      expect(isDemoSerial('JS-2C14AA'), isFalse);
      expect(isDemoSerial(null), isFalse);
    });
  });
}
