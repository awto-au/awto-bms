...
void main() {
  group('Settings status line (raw logging OFF is stated plainly)', () {
    ...
    test('ON: total across every file + the rotation size (#111)', () {
      expect(
          RawLogger.statusLine(
              enabled: true,
              ...
              lastWrittenAt: null),
          'Logging — 3.0 MB in 1 file (new file every 50.0 MB, all kept)');
      expect(
          RawLogger.statusLine(
              enabled: true,
              sizeBytes: 3 * 1024 * 1024,
              maxBytes: RawLogger.defaultMaxBytes,
              lastWrittenAt: null,
              rotatedBytes: 100 * 1024 * 1024,
              rotatedCount: 2),
          'Logging — 103.0 MB in 3 files (new file every 50.0 MB, all kept)');
    });
  ...
  group('log entry points produce lines', () {
    ...
    test('a TX line records the command name and bytes', () {
      RawLogger.instance.logTx('JS-1', 'handshake begin', [0xFB, 0xC8, 0x7C]);
