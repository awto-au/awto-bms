...
///  * [FakeKnownStore] — in-memory [KnownBatteryStore] (#110).
library;
...
/// In-memory [KnownBatteryStore] fake (#110).
class FakeKnownStore implements KnownBatteryStore {
  KnownBatteries saved = const KnownBatteries();
  int saveCount = 0;

  @override
  Future<KnownBatteries> load() async => KnownBatteries(
      seeded: saved.seeded,
      batteries: Map<String, KnownBattery>.from(saved.batteries));

  @override
  Future<void> save(KnownBatteries known) async {
    saveCount++;
    saved = KnownBatteries(
        seeded: known.seeded,
        batteries: Map<String, KnownBattery>.from(known.batteries));
  }
}
