...
void main() {
  ...
  group('#70 seriesStats — current / max / min / time-weighted median', () {
    ...
    test(
        '#117 statsText renders now/min/max/med/avg (the user\'s order) '
        'through the °F transform', () {
      // 25 held 100 ms, 43 held 200 ms -> avg (2500 + 8600) / 300 = 37.0.
      final st = seriesStats([iv(25, 0, 100), iv(43, 100, 300)], 0, 300)!;
      expect(statsText(st),
          'now 43.0 · min 25.0 · max 43.0 · med 43.0 · avg 37.0');
      // 43 °C -> 109.4 °F, rendered with fmtStat's >= 100 -> 0 dp rule;
      // the 37 °C average -> 98.6 °F.
      expect(statsText(st, celsiusToFahrenheit),
          'now 109 · min 77.0 · max 109 · med 109 · avg 98.6');
      final cells = seriesStats([iv(3.0, 0, 100), iv(3.3, 100, 1000)], 0, 1000)!;
      expect(statsText(cells),
          'now 3.300 · min 3.000 · max 3.300 · med 3.300 · avg 3.270');
    });
  });

  // -------------------------------------------------------------------------
  group('#117 seriesStats — time-weighted average', () {
    test('unequal durations: each value counts for how long it was held', () {
      // 3.0 held 900 ms, 4.0 held 100 ms -> 3.1 (a plain mean would be 3.5).
      final st = seriesStats([iv(3.0, 0, 900), iv(4.0, 900, 1000)], 0, 1000)!;
      expect(st.average, closeTo(3.1, 1e-12));
    });

    test('a brief spike barely moves the average', () {
      // 10 held an hour, a 1 s spike to 100.
      final st = seriesStats([
        iv(10, 0, 3600000),
        iv(100, 3600000, 3601000),
      ], 0, 3601000)!;
      expect(st.average, closeTo((10 * 3600000 + 100 * 1000) / 3601000, 1e-9));
      expect(st.average, lessThan(10.03));
      expect(st.max, 100);
    });

    test('an offline gap is never averaged across (only held time counts)',
        () {
      // 2.0 held 0..100, OFFLINE 100..900, 6.0 held 900..1000. The gap adds
      // no weight: 50/50 -> 4.0, not a value bridged or held through the gap.
      final st = seriesStats([iv(2.0, 0, 100), iv(6.0, 900, 1000)], 0, 1000)!;
      expect(st.average, closeTo(4.0, 1e-12));
    });

    test('rows straddling BOTH window edges count only their clipped part', () {
      // 1.0 held 0..1000 (window from 900 -> 100 ms), 5.0 held 1000..1200
      // (200 ms), 9.0 held 1200..5000 (window to 1300 -> 100 ms).
      // (1*100 + 5*200 + 9*100) / 400 = 5.0.
      final st = seriesStats([
        iv(1, 0, 1000),
        iv(5, 1000, 1200),
        iv(9, 1200, 5000),
      ], 900, 1300)!;
      expect(st.average, closeTo(5.0, 1e-12));
      // Unclipped the 9.0 row would dominate.
      final all = seriesStats([
        iv(1, 0, 1000),
        iv(5, 1000, 1200),
        iv(9, 1200, 5000),
      ], 0, 5000)!;
      expect(all.average, closeTo((1000 + 1000 + 9 * 3800) / 5000, 1e-12));
    });

    test('a single row: the average is its value', () {
      final st = seriesStats([iv(3.25, 0, 500)], 0, 1000)!;
      expect(st.average, 3.25);
      expect(st.median, 3.25);
      expect(st.min, 3.25);
      expect(st.max, 3.25);
    });

    test('a single just-opened (zero-length) row: the average is its value',
        () {
      final st = seriesStats([iv(7.5, 400, 400)], 0, 1000)!;
      expect(st.average, 7.5);
    });

    test('all zero-length rows fall back to the plain mean', () {
      final st = seriesStats([iv(7, 10, 10), iv(3, 20, 20), iv(5, 30, 30)],
          0, 100)!;
      expect(st.average, 5.0);
    });

    test('non-numeric rows are ignored by the average', () {
      final st = seriesStats([
        iv(2, 0, 100),
        const ReadingInterval(
            metric: 'm', valueText: 'x', startMs: 100, endMs: 900),
        iv(4, 900, 1000),
      ], 0, 1000)!;
      expect(st.average, closeTo(3.0, 1e-12));
    });
  });
  ...
  group('#70 ChartCard — stats row + per-card axis toggle', () {
    ...
    testWidgets(
        'shows now/min/max/med/avg for the window in the series colour',
        (tester) async {
      await tester.pumpWidget(card());
      final f = find
          .text('now 3.300 · min 3.000 · max 3.300 · med 3.300 · avg 3.270');
    ...
    testWidgets('several series: one labelled line per series',
      ...
      expect(
          find.text('Cell 1: now 3.300 · min 3.000 · max 3.300 · '
              'med 3.300 · avg 3.270'),
          findsOneWidget);
      expect(
          find.text('Cell 2: now 3.100 · min 3.100 · max 3.100 · '
              'med 3.100 · avg 3.100'),
          findsOneWidget);
    });

    testWidgets('#117 each series stays on ONE line, even on a narrow phone',
        (tester) async {
      ...
      for (final label in ['Cell 1: ', 'Cell 2: ']) {
        final f = find.textContaining(label);
        expect(f, findsOneWidget);
        expect(tester.widget<Text>(f).maxLines, 1);
        // Scaled to fit rather than overflowing or wrapping.
        expect(tester.getRect(f).right, lessThanOrEqualTo(320.5));
      }
    });
