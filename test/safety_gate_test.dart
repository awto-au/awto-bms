...
void main() {
  ...
  group('H3: disconnect alarm only on a true connected -> disconnected drop',
      () {
    test('a drop from connected alarms (red + reason) WITHOUT a beep',
        () async {
      final t = FakeTransport();
      final c = BatteryConnection(transport: t);
      await c.connectTo('dev-1', name: 'JS-A');
      ...
      expect(c.consumeBeep(), isFalse, reason: 'a lost link never beeps');
      expect(c.alarmReasons.last, contains('Disconnected'));
    });

    test('a genuine fault edge still beeps (and a drop does not)', () async {
      final t = FakeTransport();
      final c = BatteryConnection(transport: t);
      await c.connectTo('dev-1', name: 'JS-A');
      // Chip over-temperature byte[0] = a genuine temperature fault.
      c.parser.addBytes([0xA6, 0xC0, 1, 0, 0, 0, 0, 0, 0, 0xB7, 0x72]);
      expect(c.hasGenuineFault, isTrue);
      expect(c.consumeBeep(), isTrue, reason: 'faults keep their beep');
      t.lastLink!.dropLink();
      await Future<void>.delayed(Duration.zero);
      expect(c.alarmReasons.last, contains('Disconnected'));
    ...
    test('a failed RE-connect attempt (connecting -> disconnected) does NOT '
      ...
      t.lastLink!.dropLink();
      await Future<void>.delayed(Duration.zero);
      expect(c.alarmActive, isTrue); // the genuine drop (silent)
      expect(c.consumeBeep(), isFalse);
