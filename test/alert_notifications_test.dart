...
void main() {
  ...
  group('desiredNotifications', () {
    ...
    test('a fleet disconnect -> the silent Connection channel', () {
      final d =
          desiredNotifications([snap('JS-1', disconnected: true)], enabled: true);
      final n = d.values.single;
      expect(n.channel, AlertChannel.connection);
      expect(n.title, contains('Disconnected'));
      expect(n.id, notificationIdFor('JS-1', AlertCondition.disconnect));
    });

    test('only the disconnect uses the silent Connection channel', () {
      final d = desiredNotifications([
        snap('JS-1', fault: true, faultReason: 'current'),
        snap('JS-2', unknownChange: true, unknownReason: 'unknownB1'),
        snap('JS-3', disconnected: true),
      ], enabled: true);
      final bySerial = {for (final n in d.values) n.payload: n.channel};
      expect(bySerial, {
        'JS-1': AlertChannel.faults,
        'JS-2': AlertChannel.alerts,
        'JS-3': AlertChannel.connection,
      });
    });
    ...
    test('a pack with several conditions -> one notification per condition', () {
      ...
      expect(d.values.map((n) => n.channel).toSet(),
          {AlertChannel.faults, AlertChannel.alerts, AlertChannel.connection});
    });
